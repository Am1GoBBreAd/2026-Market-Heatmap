...
def load_api_key_from_dotenv(dotenv_path: Path = Path(".env")) -> str | None:
...
def build_openai_client(use_mock: bool, api_key: str | None) -> OpenAI | None:
    if use_mock:
        return None

    resolved_api_key = (api_key or os.getenv("OPENAI_API_KEY", "") or load_api_key_from_dotenv() or "").strip()
    if not resolved_api_key:
        raise RuntimeError(
            "Missing OpenAI API key. Set OPENAI_API_KEY or pass --api-key. "
            "If you only want to test the workflow, run with --mock."
        )
...
def run_pipeline(
    ...
    client = build_openai_client(use_mock=use_mock, api_key=api_key)
...
def main() -> None:
    ...
    parser.add_argument("--api-key", type=str, default=None, help="Optional OpenAI API key (overrides env var)")
